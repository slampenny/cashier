...
use Carbon\Carbon;
/*use Gateway\Gateway;
use Gateway\Charge as GatewayCharge;
use Gateway\Invoice as GatewayInvoice;
use Gateway\Customer as GatewayCustomer;
use Gateway\Error\Card as GatewayErrorCard;
use Gateway\Error\InvalidRequest as GatewayErrorInvalidRequest;*/
...
class PaymentGateway
{
    ...
    /**
     * Create a new Gateway gateway instance.
     *
    ...
    public function create($token, array $properties = [], $customer = null)
    {
        ...
        $this->billable->setGatewaySubscription(
    ...
    {
        $customer = $this->getGatewayCustomer();
        ...
        // that should be the expected behavior for most developers with Gateway.
    ...
    {
        $this->noProrate()->skipTrial()->create($token, [], $this->getGatewayCustomer());
    ...
    public function invoice()
    {
        try {
            ...
            GatewayInvoice::create(['customer' => $customer->id], $this->getGatewayKey())->pay();

            return true;
        } catch (GatewayErrorInvalidRequest $e) {
            return false;
        }
    }
    ...
    {
        try {
            return new Invoice($this->billable, GatewayInvoice::retrieve($id, $this->getGatewayKey()));
        } catch (Exception $e) {
            //
        }
    }
    ...
    public function invoices($includePending = false, $parameters = [])
    {
        ...
        $stripeInvoices = $this->getGatewayCustomer()->invoices($parameters);

        // Here we will loop through the Gateway invoices and create our own custom Invoice
        // instances that have more helper methods and are generally more convenient to
        // work with than the plain Gateway objects are. Then, we'll return the array.
    ...
    public function upcomingInvoice()
    {
        try {
            ...
            $stripeInvoice = GatewayInvoice::upcoming(['customer' => $customer->id]);
     ...
     *
     * @param  int  $quantity
     * @param  \Gateway\Customer|null  $customer
     * @return void
     */
    public function updateQuantity($quantity, $customer = null)
    {
        $customer = $customer ?: $this->getGatewayCustomer();
    ...
    public function cancel($atPeriodEnd = true)
    {
        ...
        if ($atPeriodEnd) {
            $this->billable->setGatewayIsActive(false)->saveBillableInstance();
        } else {
            $this->billable->setSubscriptionEndDate(Carbon::now());

            $this->billable->deactivateGateway()->saveBillableInstance();
        }
    }
    ...
    public function updateCard($token)
    {
        ...
        $this->billable
             ->setLastFourCardDigits($this->getLastFourCardDigits($this->getGatewayCustomer()))
    ...
    public function updateLocalGatewayData($customer, $plan = null)
    {
        $this->billable
                ...
                ->setGatewayPlan($plan ?: $this->plan)
                ->setLastFourCardDigits($this->getLastFourCardDigits($customer))
                ->setGatewayIsActive(true)
                ->setSubscriptionEndDate(null)
                ->saveBillableInstance();
    }

    /**
     * Create a new Gateway customer instance.
     *
     * @param  string  $token
     * @param  array   $properties
     * @return \Gateway\Customer
     */
    public function createGatewayCustomer($token, array $properties = [])
    {
        if ($this->coupon) {
            $properties['coupon'] = $this->coupon;
        }

        $customer = GatewayCustomer::create(
            array_merge(['source' => $token], $properties), $this->getGatewayKey()
        );

        return $this->getGatewayCustomer($customer->id);
    }
    ...
    protected function usingMultipleSubscriptionApi($customer)
    {
        return ! isset($customer->subscription) &&
                 ...
                 ! is_null($this->billable->getGatewaySubscription());
    }
     ...
     *
     * @param  string  $coupon
     * @return \Laravel\Cashier\PaymentGateway
     */
    ...
    /**
     * Get the trial end timestamp for a Gateway subscription update.
     *
     * @return int
     */
    ...
    public function maintainTrial()
    {
        if ($this->billable->readyForBilling()) {
            if (! is_null($trialEnd = $this->getTrialEndForCustomer($this->getGatewayCustomer()))) {
